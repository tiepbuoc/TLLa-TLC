...
class AIInputMethod(InputMethod):
    location = "<imethod.tllai.AIInputMethod>"
    def __init__(
        self, 
        *, 
        vni_tones=False, 
        ...
        **kwargs
        ...
        self.mode = "[AI]"
        self.SOS = "tôi" # đầu của chuỗi
        
        if verbose:
            print(f"{AIInputMethod.location} Đang tải...")
    ...
    def accept(self, crt: Triplet, crst: MatchingTriplet, word: Word, vni_tones=False):
        ...
        consonant, rhyme, tone = crt.unpack() # Triplet của từ được dự đoán bởi AI
        consonant_rule, rhymes_rule, tone_rule = crst.unpack() # Khả năng từ người dùng
        
        if self.vni_tones or vni_tones:
            # Vẫn chấp nhẫn tone=6 hoặc tone=7 mặc dù tone_rule là 1 hoặc 5
