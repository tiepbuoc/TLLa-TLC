...
class InputMethod():
    location = "<imethod.tlla.InputMethod>"
    def __init__(
        self, 
        *, 
        vni_tones=False, 
        ...
        **kwargs
        ...
        self.vni_tones = vni_tones        # Dấu cấp cao cho cả tone 1 và 6, dấu chấm dưới cho cả tone 5 và 7
        self.strict_k = strict_k
        self.null_consonant = null_consonant # Hiện hỗ trợ chỉ 1 hoặc 2 ký tự.
        self.end_of_rhyme = end_of_rhyme # `co5ve4tr0` không thể cho ra `cọ vẽ tranh`, và không thể nhập `cọ vẽ tranh` bằng CASE 4 vì `cọ` không thường dùng. `end_of_rhyme` được tạo để sửa điều này: `co{EOR}5ve4tr0`. Hiện hỗ trợ chỉ 1 ký tự.
        
    def parse(self, crt: Raw) -> RawTriplet:
        '''
        Bộ nhận biết chuỗi đầu vào (cvt). Nhận biết 3 phần trong chuỗi: phụ âm, vần, và tông. 
        Trả về tuple `(consonant_part, rhyme_part, tone_part)`.
        '''
        crt = crt.lower()
        # KIỂM TRA TÔNG
        try:
            ...
            if tone not in Vietnamese.tones:
                return None
            #
            # TODO: Thêm nếu tone == 6, 7 cho vni_tones
            #
        except Exception:
            return None
        
        # KIỂM TRA PHỤ ÂM   
        # Kiểm tra xem phụ âm rỗng có được sử dụng
        if crt[:2] == self.null_consonant: consonant, rhyme_pos = self.null_consonant, 2
            ...
            # Kiểm tra xem phụ âm có là phụ âm kép
            consonant = crt[:2]
            rhyme_pos = 2
            if consonant not in Vietnamese.consonant_families + ['dd']:
                # Kiểm tra xem phụ âm có là phụ âm đơn
                consonant = crt[:1]
                rhyme_pos = 1
    ...
    def find(self, raw_triplet: RawTriplet) -> MatchingTriplet:
        '''
        Nhẫn 3 phần của raw: phụ âm, vần, và tông. Thực vần có thể rỗng, trong trường hợp này vần sẽ là tất cả vần trong Vietnamese.rhyme_families (kự dằng).
        
        Trả về `MatchingTriplet`.
        '''
        IS_Q_CASE = False
        if not raw_triplet:
            return None
        ...
        if consonant in ['c', 'q']:
            if self.strict_k:
                return None
            # Người dùng có thể nhập qua/que thay vì coa/coe -> thay thế `u` bằng `o`
            if consonant == 'q':
                IS_Q_CASE = True
    ...
    def rhymeRecognizer(self, raw_rhyme: str, is_q_case = False) -> List[RhymeFamily]:
        '''
        Trả về những vần khậb với thực vần (rh).
        '''
        if is_q_case:
