...
class Vietnamese(Alphabet):
    ...
    type quin/quịt, qui/quí in tlla. (see Vietnamese.synthesize)
    """
    ...
    def diacritic(word: Word):
        """Trả về số đặc chỉ mục (diacritic number) của một từ, chỉ số trong từ tại đó đặc chỉ mục xảy ra.
        Lưu ý rằng đặc chỉ mục khác với tông, và diacritic ở đây là các dấu liên quan đến tông
        (dấu cấp cao, hạ thấp, vũ, nšu hạ văn, hoặc đầu chấm dưới), không phải các dấu như â, ă, ư, ...
        
        Các giá trị diacritic:
        - 0: Không có dấu (trả về (0, None, None))
        - 1: Dấu cấp cao (tone 1 hoặc 6)
        - 2: Dấu hạ (tone 2)
        - 3: Dấu vũ (tone 3)
        - 4: Dấu nšu (tone 4)
        - 5: Dấu đầu chấm dưới (tone 5 hoặc 7)
        
        Nhiều dấu = không phải từ Việt = trả về (None, None, None)
        """
    ...
    def analyze(word: Word) -> Union[Triplet, str]:
        """
        Vui lòng sử dụng từ viết thường.
        Trả về họ phụ âm, họ vần, và tông của từ. Nếu từ không được nhận diện là từ Tiếng Việt, trả về word thay thế.
        - Họ phụ âm: c, k, ng, g, z (gi), 0 (rỗng) thuộc Vietnamese.consonant_families
        - Họ vần: a, e, i, o, u, ... (cực kỳ đa dạng với các vần kết hợp) thuộc Vietnamese.rhymes_families
        - Tông: 0-7 (tông đặc trưng của tiếng Việt)
        """
        original_word = word
