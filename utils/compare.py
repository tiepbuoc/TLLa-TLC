from imethod.tlla import InputMethod
...
class TLLa():
    ...
    def get_least_from_words(words: list[str]):
        ...
        if len(words) == 1:
            ...
            if cf == '0':
                return TLLa.get_full_from_crt((cf, rf, t))
            else:
                ...
                res = TLLa.inputAgent.predict(relative_raw)
                relative_raw
                '''
        elif len(words) <= 3:            
            ...
            total = 0
            for crt in crts:
                total += TLLa.get_full_from_crt(crt)
    ...
    def get_keys_needed_from_word(word: str):
        word = word.lower()
        return TLLa.get_keys_needed_from_crt(Vietnamese.analyze(word))
    ...
    def get_keys_needed_from_crt(crt: tuple[str, str, str]):
        ...
        return TLLa.get_full_from_crt(crt)
...
def main1():
    ...
    for word in corpus.split(' '):
        print(word, TelexOrVNI.get_keys_needed_from_word(word), TLLa.get_keys_needed_from_word(word))
...
def compare():
    ...
    for idx, phrase in enumerate(samples, 1):
        ...
        try:
            if len(words) == 1:
                total_tlla_method = TLLa.get_least_from_words(words) + 1 + 1 # +1 page because many words appear, +1 for choosing key
            else:
                total_tlla_method = TLLa.get_least_from_words(words) + 1 # +1 for choosing key
        except Exception:
            continue
        ...
        records.append((phrase, total_other_input_method, total_tlla_method))
        if len(words) == 1:
            records_len_1.append((phrase, total_other_input_method, total_tlla_method))
        elif len(words) == 2:
            records_len_2.append((phrase, total_other_input_method, total_tlla_method))
        elif len(words) == 3:
            records_len_3.append((phrase, total_other_input_method, total_tlla_method))
        else:
            records_len_4.append((phrase, total_other_input_method, total_tlla_method))
            
        print(f'\r{idx}/{len_samples}', end='')
        
    diffs = [other - tlla for _, other, tlla in records]
    faster_rate = [(other - tlla)/other for _, other, tlla in records]
    
    diffs_len_1 = [other - tlla for _, other, tlla in records_len_1]
    faster_rate_len_1 = [(other - tlla)/other for _, other, tlla in records_len_1]
    diffs_len_2 = [other - tlla for _, other, tlla in records_len_2]
    faster_rate_len_2 = [(other - tlla)/other for _, other, tlla in records_len_2]
    diffs_len_3 = [other - tlla for _, other, tlla in records_len_3]
    faster_rate_len_3 = [(other - tlla)/other for _, other, tlla in records_len_3]
    diffs_len_4 = [other - tlla for _, other, tlla in records_len_4]
    faster_rate_len_4 = [(other - tlla)/other for _, other, tlla in records_len_4]
