...
class Dictionary():
    ...
    def get(triplets: List[Union[Triplet, MatchingTriplet]], max=25, freq_threshold=2) -> Optional[List[List[Word]]]:
        '''
        Trả về các khả năng từ của từng bộ số cho danh sách bộ số: `list[tuple[consonant: str, rhyme: str | list[str] | 'any', tone: str]]`
        
        `rhyme` có thể là một vần, hoặc danh sách vần, hoặc `any` cho tất cả vần có thể.
        
        Tham số:
        - `max`: n từ phổ biến nhất trên mổi bộ số (mặc định là 25).
        - `freq_threshold`: chỉ lấy những từ có tần suất cao hơn hoặc bằng ngưỡng này.
        '''
    ...
    def predict(words_possibilities: List[List[Word]], verbose=False, any=False) -> List[Phrase]:
        '''
        Có thể chưa tối ưu hóa.
        Nhẫn danh sách các khả năng từ (`List[List[Word]]`) rồi tạo tất cả cổ hợp n-gram với thứ tự của danh sách.
        
        Sau đó lọc ra các cổ hợp nào có trong từ điển.
        Đặt `any` thành True sẽ bỏ qua phần lọc.
        '''
        if verbose:
            lens = []
...
if __name__ == "__main__":
    '''
    Vui lòng bỏ qua, đây là để cập nhật db.json và dict.json từ thư mục dữ liệu riêng tư của tôi.
    '''
    if len(sys.argv) < 3:
        pass
    else:
